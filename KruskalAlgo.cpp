...
public:
    ...
    explicit DisjointSets(int size) : size(size), max_table_size(0), sets(size)
    {
        ...
        {                       // makeSet(i) operation, i.e. create a singleton set
    ...
    int getParent(int i)
    { // similar to Find(). Find parent and compress path. [This will later allow log*()
        if (i != sets[i].parent)
        { //time as the tree depth does not increase much by compression]
            sets[i].parent = getParent(sets[i].parent);
        }
        return sets[i].parent;
    }
    void merge(int i, int j)
    { // merging or union of two sets (union_rank) - this makes the algorithm log*() complexity
    ...
    int findNumberofUniqueParents()
    { //Not a classical disjoint set or Kruskal algorithm component
...
double clustering(vector<int> x, vector<int> y, int desiredNoClusters)
{
    ...
    if (bVerbose)
    {
        displayVectorofPair(distVector, "Dist Point Pair Vector:");
    }
    ...
    if (bVerbose)
    {
        displayVectorofPair(distVector, "Dist Point Pair Vector (Sorted):");
    }
    ...
    for (size_t i = 0; i < distVector.size(); ++i)
    {
        ...
        { //do merge only if they are not connected already
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < argc; ++i)
    {
        ...
        {
            cout << "Verbose option is requested" << endl;
            bVerbose = true;
        }
    }
    size_t n;
