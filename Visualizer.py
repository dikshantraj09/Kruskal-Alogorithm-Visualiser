...
def FindFirstandLastCost(xpoints, ypoints, parentlines, nParents):
    ...
    for i in range(1, nParents):
        cost_first += sqrt((xpoints[i]-xpoints[parents[i]])
                           ** 2 + (ypoints[i]-ypoints[parents[i]])**2)
    ...
    cost_last = 0
    for i in range(1, nParents):
        cost_last += sqrt((xpoints[i]-xpoints[parents[i]])
                          ** 2 + (ypoints[i]-ypoints[parents[i]])**2)
...
def main():
    ...
    # This mean 16x9 inches (number of pixels is (16x9)*dpi value set in savefig
    fig2 = plt.figure(5, figsize=(16, 9))
    ...
    for k in range(0, nConnectedPoints):
        ...
        plt.grid('on')
        plt.title(r'Cost: $\sum_{\forall\, i,j} (P_{i}-P_{j})_{connected}$ = ' +
                  str(int(cost_k*10)/10.0), fontsize=15, y=1.02)
...
def ParseInput():
    ...
    parser.add_argument("-input", type=str, default=None,
                        help="enter original input")
    parser.add_argument("-connectedpoints", type=str,
                        default=None, help="Enter cost and distance file")
